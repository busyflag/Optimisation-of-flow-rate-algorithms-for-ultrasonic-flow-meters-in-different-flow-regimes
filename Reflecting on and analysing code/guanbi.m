...
% 流速计算公式：流速 = 流量 / 时间。这里我们假设单位时间。
flow_rate_traditional = Q_traditional; % 使用传统方法计算的流量
flow_rate_new = Q_new; % 使用新方法计算的流量

% 流速 = 流量 / 时间，假设单位时间
v_speed_traditional = flow_rate_traditional / (pi * R^2);
v_speed_new = flow_rate_new / (pi * R^2);
...
% 5. 流速对比图（常数流速）
subplot(5,1,5);
plot([0, R], [v_speed_traditional, v_speed_traditional], 'r', 'LineWidth', 2); % 传统方法流速
hold on;
plot([0, R], [v_speed_new, v_speed_new], 'b', 'LineWidth', 2); % 新方法流速
title('流速对比：传统方法 vs 新方法（常数流速）');
xlabel('半径 r(m)');
ylabel('流速 u(m/s)');
legend('传统方法流速', '新方法流速');
grid on;
